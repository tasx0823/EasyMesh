...
def bottleneck(inputs,
  ...
  with variable_scope.variable_scope(scope, 'bottleneck_v2', [inputs]) as sc:
    ...
    preact = tf.nn.relu(tflearn.batch_normalization(inputs,scope='preact'))
  ...
  with variable_scope.variable_scope(
    ...
    with arg_scope(
      ...
      with arg_scope([layers.batch_norm], is_training=is_training):
        ...
        net = tf.nn.relu(tflearn.batch_normalization(net,scope='postnorm',decay=0.997))
        if global_pool:
        ...
        net = tf.squeeze(net,axis=[1,2],name='squeezed')
        if num_classes is not None:
